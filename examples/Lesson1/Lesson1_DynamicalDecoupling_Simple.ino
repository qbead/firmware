...
This is the SIMPLIFIED version of the Dynamical Decoupling experiment, where the vertical magnetic field axis is fixed to
90-degree increments of the QBead. This means that slight variations in the QBead's orientation will not interrupt the DD
procedure.
...
- angle_round: the angle increment to round the magnetic axis to
...
int reset_time=10000;       // Length of each reset sequence
int reset_delay=800;        // Length of reset delay - how long it blinks for to indicate reset to new sequence

float omega = -0.005;       // Base rotation (precession) per loop iteration of the ensemble states
...
float angle_round = 90.; // The angle increments to round the orientation of the vertical magnetic field axis to
  ...
  */
  ...
  if (t<reset_time) {
    ...
    */
    ...
    // Initialise global vertical axis as AngleVec (needed for precession calculations) - round to nearest increment of angle_round
    AngleVec axis;
    axis.theta=(angle_round*round(bead.t_acc/angle_round));
    axis.phi=(angle_round*round(bead.p_acc/angle_round));
