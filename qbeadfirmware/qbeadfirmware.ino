...
#define DEFAULT_COLOR {255,0,255}
...
namespace Qbead
{
  ...
  public:
    Qbead(const uint16_t pin00 = QB_LEDPIN,
          ...
          const bool sx = QB_SX,
          const bool sy = QB_SY,
          const bool sz = QB_SZ) : imu(LSM6DS3(I2C_MODE, imu_addr)),
...
void loop()
{
  ...
  char col[3];
  bead.blecharcol.read(&col, 3);
  bead.setBloch_deg_smooth(bead.t, bead.p, color(col[0], col[1], col[2]));
  bead.show();
  delay(10);
}
