...
const uint8_t QB_UUID_MEAS_CHAR[] =
{0x45,0x8d,0x08,0xaa,0xd6,0x63,0x44,0x25,0xbe,0x12,0x9c,0x35,0xc6-1,0x1f,0x0c,0xe3};
...
public:
  ...
  void begin() {
    ...
    blecharmeas.setProperties(CHR_PROPS_READ | CHR_PROPS_NOTIFY);
    blecharmeas.setPermission(SECMODE_OPEN, SECMODE_OPEN);
    blecharmeas.setUserDescriptor("measurement");
    blecharmeas.setFixedLen(1*sizeof(int));
    blecharmeas.begin();
    blecharmeas.write(zerobuffer20, 1*sizeof(int));
    startBLEadv();
  }
  ...
  void sendMeasurement(int meas) {
    ibuffer[0] = meas;
    blecharmeas.write(ibuffer, 1*sizeof(int));
    for (uint16_t conn_hdl=0; conn_hdl < QB_MAX_PRPH_CONNECTION; conn_hdl++)
    {
      if ( Bluefruit.connected(conn_hdl) && blecharmeas.notifyEnabled(conn_hdl) )
      {
        blecharmeas.notify(ibuffer, 1*sizeof(int));
      }
    }
  }
